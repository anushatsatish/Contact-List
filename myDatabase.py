...
def create_table():
    ...
    conn.execute('''CREATE TABLE test_lab8
...
def update_table(c_name, c_number,o_name,o_number):
    ...
    try:
        conn = sqlite3.connect('contacts.db')
        update_cmd = "UPDATE test_lab8 SET NAME = '%s',CONTACT= '%s'  where NAME = '%s' AND CONTACT = '%s'" %(c_name, c_number, o_name, o_number)
        conn.execute(update_cmd)
        conn.commit
        print("Total number of rows updated :", conn.total_changes)
        print("Successfully updated %s - %s with new details %s - %s" % (o_name, o_number, c_name, c_number))
        cursor = conn.execute("SELECT name, CONTACT from test_lab8")
        for row in cursor:
...
def delete_records(c_name):
    ...
    conn = sqlite3.connect('contacts.db')
    delete_cmd = "DELETE from test_lab8 where NAME = '%s';"%c_name
...
def insert_records(c_name, c_number):
    ...
    try:
        conn = sqlite3.connect('contacts.db')
        insert_cmd = "INSERT INTO test_lab8 (NAME,CONTACT)VALUES ('%s','%s')"%(c_name, c_number)
